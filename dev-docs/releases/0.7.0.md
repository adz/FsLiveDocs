...
- Verify all snapshot examples from one project in a shared FSI session instead of retaining a compiler session per example. Share one concurrency-safe `FSharpChecker` across parallel documentation checks and release completed check-result graphs before snapshot execution. Large captures now keep compiler memory bounded and finish explicit verification substantially faster.
