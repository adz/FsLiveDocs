...
module View =
    ...
    let sidebarPageLink (p: ContentPage) =
        let fileName = Path.GetFileNameWithoutExtension(p.FilePath).ToLower() + ".html"
        li [] [ a [ _href $"/{fileName}"; _class "py-2 px-4 hover:bg-base-300 rounded-lg block text-sm" ] [ str p.Metadata.Title ] ]

    let rec sidebarEntityLink (e: EntityModel) =
        li [] [
            details [ _class "group"; attr "open" "true" ] [
                summary [ _class "flex items-center justify-between py-2 px-4 hover:bg-base-300 rounded-lg cursor-pointer list-none" ] [
                    span [ _class "text-sm font-medium" ] [ 
                        a [ _href $"/api.html#{e.Id}"; _class "hover:link" ] [ str e.Name ]
                    ]
                    i [ _class "bi bi-chevron-down text-[10px] transition-transform group-open:rotate-180" ] []
                ]
                (if not e.Entities.IsEmpty then
                    ul [ _class "menu menu-sm pl-4 pt-1" ] (e.Entities |> List.map sidebarEntityLink)
                else emptyText)
            ]
        ]

    let sidebar (pages: ContentPage list) (package: PackageModel) =
        div [ _class "flex flex-col gap-6" ] [
            // Guides Section
            div [] [
                h3 [ _class "text-xs font-black uppercase opacity-40 px-4 mb-2 tracking-tighter" ] [ str "Guides" ]
                ul [ _class "menu menu-sm p-0 gap-1" ] (pages |> List.sortBy (fun p -> p.Metadata.Weight) |> List.map sidebarPageLink)
            ]
            // API Section
            div [] [
                h3 [ _class "text-xs font-black uppercase opacity-40 px-4 mb-2 tracking-tighter" ] [ str "API Reference" ]
                ul [ _class "menu menu-sm p-0 gap-1" ] (package.Entities |> List.map sidebarEntityLink)
            ]
        ]
    ...
    let apiCard (memberModel: MemberModel) =
        div [ _class "card bg-base-100 shadow-sm border border-base-300 overflow-hidden"; _id memberModel.Id ] [
            ...
            div [ _class "p-6" ] [
                h2 [ _class "text-2xl font-black mb-4 flex items-center gap-2" ] [ 
                ...
                (if not memberModel.Parameters.IsEmpty then
                    div [ _class "mb-6" ] [
                        h4 [ _class "text-xs font-black uppercase opacity-40 mb-3" ] [ str "Parameters" ]
                ...
                div [ _class "mb-6" ] [
                    h4 [ _class "text-xs font-black uppercase opacity-40 mb-2" ] [ str "Returns" ]
                    code [ _class "text-accent font-mono text-xs" ] [ str memberModel.ReturnType ]
                ]

                (if not memberModel.Examples.IsEmpty then
                    div [] [
                        h4 [ _class "text-xs font-black uppercase opacity-40 mb-3" ] [ str "Verification Examples" ]
                        (memberModel.Examples |> List.map (fun ex ->
                            div [ _class "mb-4" ] [
                                ...
                                pre [ _class "bg-neutral text-neutral-content p-4 rounded-xl text-xs font-mono overflow-x-auto" ] [
    ...
    let layout (pageTitle: string) (pages: ContentPage list) (package: PackageModel) (versions: string list) (theme: string) (content: XmlNode list) =
        html [ _lang "en"; attr "data-theme" theme; _class "scroll-smooth" ] [
            ...
            body [ _class "min-h-screen bg-base-200/50 flex flex-col" ] [
                // Navbar
                div [ _class "navbar bg-base-100 border-b border-base-300 sticky top-0 z-[100] h-16" ] [
                    ...
                    div [ _class "flex-1 px-4" ] [
                        a [ _href "/"; _class "flex items-center gap-2 no-underline" ] [ 
                ...
                div [ _class "flex-1 flex overflow-hidden" ] [
                    div [ _class "drawer lg:drawer-open" ] [
                        ...
                        div [ _class "drawer-content flex flex-col h-full overflow-y-auto" ] [
                            div [ _class "max-w-7xl w-full mx-auto p-6 lg:p-12" ] [
                                div [ _class "grid grid-cols-1 xl:grid-cols-4 gap-12" ] [
                                    main [ _class "xl:col-span-3 prose prose-sm md:prose-base max-w-none bg-base-100 p-8 md:p-12 rounded-3xl shadow-sm border border-base-300" ] [
                                    ...
                                    aside [ _class "hidden xl:block" ] [
                                        div [ _class "sticky top-28" ] [
                                            ...
                                            ul [ _class "menu menu-xs opacity-70 border-l-2 border-base-300 gap-1 ps-4" ] [
                                                li [] [ a [ _href "#"; _class "font-bold" ] [ str "Introduction" ] ]
                        ...
                        div [ _class "drawer-side z-50 h-full sticky top-16" ] [
                            ...
                            div [ _class "bg-base-100 w-72 h-[calc(100vh-4rem)] border-r border-base-300 overflow-y-auto p-6" ] [
                                sidebar pages package
                            ]
                        ]
                    ]
                ]
